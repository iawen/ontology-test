...
const REVIEW_LABELS = { approved: "通过", pending: "待审", rejected: "不通过" } as const;
const getReviewStatus = (item?: Partial<Pick<Concept, "is_reviewed" | "review_status">> | null) => item?.review_status || (item?.is_reviewed ? "approved" : "pending");
      ...
      {nodes.map(node => (
        <div key={node.id} className="py-1">
          <div className="flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-slate-50 group">
            ...
            <span className={`text-xs px-1.5 py-0.5 rounded ${REVIEW_BADGE[getReviewStatus(node)]}`}>{REVIEW_LABELS[getReviewStatus(node)]}</span>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setEditConcept({ scenario_id: activeScenario, parent_id: "", level: 0, concept_type: "entity", related_class: "", sort_order: 0, is_reviewed: false, review_status: "pending" }); setIsModalOpen(true); }} className="btn-primary">+ 新增概念</button>
        </div>
      </div>
      <ScenarioSelector />
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditConcept(null); }} title={editConcept?.id ? "编辑概念" : "新增概念"} footer={<><button onClick={() => { setIsModalOpen(false); setEditConcept(null); }} className="btn-outline">取消</button><button onClick={save} className="btn-primary">保存</button></>}>
        <div className="space-y-4">
          ...
          <div><label className="text-xs text-slate-500 font-medium block mb-1.5">人工审核</label><select value={getReviewStatus(editConcept)} onChange={(e) => setEditConcept({ ...editConcept!, review_status: e.target.value as Concept["review_status"], is_reviewed: e.target.value === "approved" })} className="w-40"><option value="pending">待审</option><option value="approved">通过</option><option value="rejected">不通过</option></select></div>
        </div>
      </Modal>
