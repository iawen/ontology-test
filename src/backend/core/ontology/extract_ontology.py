...
核心重构亮点：
  ...
  2. 修复引用悬空：在类合并去重时，动态构建 `class_id_map` 拓扑映射，反向刷新所有指标的 `target_class`。
    3. 第二阶段仅基于已合并的全局 Class、字段和关系线索生成结构化 Metric definition。
    4. 第三阶段在 Metric 与 Relationship 已确定后，构建 `subject_domain / dimension_group / fact_group` 概念层级。
"""
...
PHASE2_GLOBAL_PROMPT = """你是企业指标语义建模专家。第一阶段已经完成所有物理表分片的合并，下面提供的是全局唯一、字段完整的实体类（Classes）和数据源目录。
现在请在全局范围内定义可执行的结构化 Metrics，并补充这些 Metrics 所需的 Class 关联关系。不要生成 Concepts；概念层级将在第三阶段单独规划。
...
6. `dimensions` 与 `required_dimensions` 必须属于 anchor_class，`required_dimensions` 是 `dimensions` 的子集。
7. Relationships 必须结合数据源目录、字段名、主键/外键线索和 Metric 依赖判断，不能只按字段名称猜测。
...
class OntologyExtractor:
    ...
    def run(
        ...
        # ---- Phase 2: 基于全局 Class 生成结构化 Metrics 和 Relationships ----
        if on_progress:
            on_progress("phase2", 1, 1, "正在基于全局实体和字段定义结构化指标与关联关系...")
        ...
        # ---- Phase 3: 基于已确定的 Class、Relationship、Metric 构建概念层级 ----
        if on_progress:
            on_progress("phase3", 1, 1, "正在根据实体、关联与指标构建多级概念层级...")
    ...
    def _extract_global_metrics_and_relationships(self, classes: list, dataset_catalog: list, business_name: str) -> dict:
        """Phase 2: derive governed structured Metrics after Class IDs and fields are final."""
