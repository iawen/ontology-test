...
from .prompt import get_skill_routing_prompt
...
_engines: dict[str, OntologyEngine] = {}
_query_engines: dict[str, DataQueryEngine] = {}
_init_lock = asyncio.Lock()


async def init_prompt(scenario_id: str) -> None:
    """Initialize the scenario-scoped ontology and data-query engines."""
    async with _init_lock:
        ...
        if scenario_id not in _query_engines:
            db_url = ""
            try:
                from modules.data_connections import get_active_connection

                active_connection = get_active_connection(scenario_id)
                if active_connection:
                    db_url = active_connection["connection_url"]
            except Exception:
                logger.warning(
                    "Unable to load active data connection; using scenario data: scenario_id=%s",
                    scenario_id,
                    exc_info=True,
                )
            _query_engines[scenario_id] = DataQueryEngine(
                _engines[scenario_id], db_connection_url=db_url
            )

        engine = _engines[scenario_id]
        logger.info(
            "Scenario engines initialized: scenario_id=%s classes=%d metrics=%d",
            scenario_id,
            len(engine.list_classes()),
            len(engine.list_metrics()),
        )


def reset_engine(scenario_id: str) -> None:
    """Discard cached scenario engines after ontology or connection changes."""
    _engines.pop(scenario_id, None)
    _query_engines.pop(scenario_id, None)
...
def metric_context_summary(metric: dict) -> str:
    ...
    return (
        ...
        f"可选维度={metric.get('dimensions') or []}; "
        f"说明={metric.get('description', '')}"
    )
...
async def route_skills(scenario_id: str, user_message: str, conversation_history: list[dict] = None) -> list[dict]:
    ...
    routing_prompt = get_skill_routing_prompt(skill_list_str, context, user_message)
