...
from configs.global_config import client2

# ============================================================
# 配置常量
# ============================================================
SAMPLE_ROWS = 2                     # 极限降采样，节省 token
WIDE_TABLE_THRESHOLD = 30           # 触发宽表纵向分片的阈值
...
def read_db_summary(db_url: str, sample_rows: int = SAMPLE_ROWS) -> list[dict]:
...
现在需要你站在全局视角，构建系统的【默认概念层级树（Concepts Hierarchy Tree）】以及类间的【关联拓扑网络（Relationships）】。
...
class OntologyExtractor:
    def __init__(self, api_key: str = "", base_url: str = "", model: str = ""):
        ...
        # self.base_url = base_url or os.getenv("OPENAI_BASE_URL", "[https://dashscope.aliyuncs.com/compatible-mode/v1](https://dashscope.aliyuncs.com/compatible-mode/v1)")
    ...
    def run(
        ...
        if db_connection_url:
            summaries = read_db_summary(db_connection_url)
        elif input_dir:
            summaries = []
        ...
        for idx, shard in enumerate(all_shards):
            ...
            result = self._extract_single_table(shard, business_name)
            if "class" in result:
                # 留存原始血缘用于物理 Mapping
        ...
        global_topology = self._extract_global_topology(merged_classes, business_name)
        ...
        print(f"Extact Summary: {len(merged_classes)} classes, {len(relationships)} relationships, , {len(all_metrics)} metrics, , {len(concepts)} concepts")
    ...
    def _extract_global_topology(self, classes: list, business_name: str) -> dict:
        compressed = []
        for c in classes:
            compressed.append({
                "id": c.get("id"),
    ...
    def _save_assets(self, schema: dict, summaries: list, output_dir: str):
        ...
        for c in schema.get("classes", []):
            ...
            mapping["classes"][cid] = {
                ...
                "primary_key": ",".join([f["physical_name"] for f in c.get("fields", []) if f.get("is_primary_key")]),
                "name_cn": c.get("name_cn", ""),
                "field_map": {f["name"]: f["physical_name"] for f in c.get("fields", [])},
                "field_types": {f["physical_name"]: f["type"] for f in c.get("fields", [])},
